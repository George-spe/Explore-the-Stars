...
translations.en.connectWalletError = 'Please install MetaMask or another Web3 wallet to connect!';
translations.en.cooldownActive = 'Airdrop cooldown active. Please wait {time} before claiming again.';
translations.en.emailSuccess = 'Your message has been sent successfully!';
...
Object.keys(translations).forEach(lang => {
    if (lang !== 'en') {
        ...
        translations[lang].connectWalletError = translations[lang].connectWalletError || translations.en.connectWalletError;
        translations[lang].cooldownActive = translations[lang].cooldownActive || translations.en.cooldownActive;
        translations[lang].emailSuccess = translations[lang].emailSuccess || translations.en.emailSuccess;
    }
});

// 合约地址和ABI - 修复了getUserInfo问题
...
function initLanguage() {
    ...
    // TP钱包DApp浏览器特殊处理
    if (window.tp && window.tp.getLanguage) {
        window.tp.getLanguage().then(lang => {
            const tpLang = lang.toLowerCase().substring(0, 2);
            if (translations[tpLang]) {
                currentLanguage = tpLang;
                localStorage.setItem('planetLanguage', currentLanguage);
                updateLanguageUI();
            }
        }).catch(err => {
            console.log('Error getting TP wallet language:', err);
        });
    } else if (window.tp && window.tp.getAppInfo) {
        // 另一种获取TP钱包语言的方式
        window.tp.getAppInfo().then(info => {
            const tpLang = info.language.toLowerCase().substring(0, 2);
            if (translations[tpLang]) {
                currentLanguage = tpLang;
                localStorage.setItem('planetLanguage', currentLanguage);
                updateLanguageUI();
            ...
            console.log('Error getting TP wallet app info:', err);
        });
    }
