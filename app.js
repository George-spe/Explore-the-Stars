...
const translations = {
    en: {
        ...
        gasEstimationError: 'Failed to estimate gas. Please try again later.',
        transactionFailed: 'Transaction failed. Please check your gas settings and try again.'
    },
    ja: {
        ...
        gasEstimationError: 'ガス見積もりに失敗しました。後でもう一度お試しください。',
        transactionFailed: 'トランザクションが失敗しました。ガス設定を確認して再試行してください。'
    },
    ko: {
        connectWallet: "지갑 연결",
        airdrop: "에어드랍",
        ...
        gasEstimationError: '가스 추정에 실패했습니다. 나중에 다시 시도해 주세요.',
        transactionFailed: '트랜잭션이 실패했습니다. 가스 설정을 확인하고 다시 시도해 주세요.'
    },
    zh: {
        connectWallet: "连接钱包",
        airdrop: "领取空投",
        ...
        gasEstimationError: 'Gas预估失败，请稍后再试。',
        transactionFailed: '交易失败，请检查Gas设置并重试。'
    }
};
...
function updateLanguage() {
    ...
    // 通知TP钱包语言变化
    if (isTPWallet && window.tp && window.tp.setLanguage) {
...
async function initWeb3() {
    try {
        // 检查是否是TP钱包环境
        if (isTPWallet && window.tp && window.tp.ethereum) {
            web3 = new Web3(window.tp.ethereum);
            window.ethereum = window.tp.ethereum; // 为了兼容性
...
function initWallet() {
    ...
    connectBtn.addEventListener('click', async () => {
        if (typeof window.ethereum === 'undefined' && !isTPWallet) {
            showAlert(
                translations[currentLanguage].connectWalletError.split(':')[0],
                translations[currentLanguage].connectWalletError,
                true
...
function initAirdrop() {
    ...
    claimBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // 估算Gas
            try {
                const gasEstimate = await contract.methods.claimAirdrop(referrer || '0x0000000000000000000000000000000000000000')
                    .estimateGas({
                        from: recipient,
                ...
                showAlert(
                    translations[currentLanguage].gasEstimationError.split(':')[0],
                    translations[currentLanguage].gasEstimationError,
                    true
                );
                return;
            }
            ...
            if (error.message.includes('revert')) {
                if (error.message.includes('Cooldown period not over')) {
                    ...
                    errorMessage = translations[currentLanguage].transactionFailed;
                }
            }
...
function initRewards() {
    ...
    claimBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // 估算Gas
            try {
                const gasEstimate = await contract.methods.claimRewards()
                    .estimateGas({
                        from: recipient
                    });
